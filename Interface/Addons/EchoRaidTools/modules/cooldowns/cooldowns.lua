...
function _Cooldowns:ADDON_LOADED(addonName)
    ...
    if not EchoCooldowns then
        ...
        _Cooldowns.OnUnitInfoUpdate = function()
                ...
                for spellID, cooldownInfo in pairs(unitCooldowns) do
                    ...
                    for i = 1, numGroups do
                        ...
                        if _Cooldowns.GetGroupSpellByID(spellID, i) then
                            if UnitExists(unit) and ((not _Cooldowns.GetIgnoreSelf(i)) or (not UnitIsUnit(unit, "player"))) then
        ...
        -- TODO: 
        -- redo backdrops
        -- test displays when anchored to unit frames

        -- Add healthstone/healthpot (combat pot?)

        -- copy/paste section settings

        
        --ARROW KEYS NAVIGATE FILTER SELECTION
        -- ICON COLOUR

        -- fix
        -- prioritise state PROPERTIES 

        -- future additions:
        -- anchor to specified frame
        -- filters spells by spec - e.g. only show wings for Holy Paladins
        -- profiles?
        -- show charges as separate displays
        -- Blacklist in spell filter section
        -- Icon "zoom"?

        -- ADD:
        -- "Use AblityX Now" message (not part of cooldowns but add it as a new module)

        --_G["EchoCooldowns"] = LibReadOnly:New(_EchoRaidTools.Cooldowns)
        _G["EchoCooldowns"] = _EchoRaidTools.Cooldowns
    end
end