...
do
	...
	function Lib:GetTaxiTripTime(node1,node2)
		...
		local node1n=Lib:FindTaxiByNodeID(node1)
		local node2n=Lib:FindTaxiByNodeID(node2)
	...
	function Lib.OnEvent(this, event, arg1,arg2)
		...
		elseif event=="UNIT_FLAGS" and arg1=="player" and ZGV and ZGV.DEV and LibRover.ready then
			...
			old_ontaxi=ontaxi
			local taxis = false -- LibRover.TaxiWhistlePredictor:FindNearestTaxis()
			if ontaxi then
				if Lib.LastTaxi and (not taxis or not taxis[1] or taxis[1][2]>=50) then taxis={{Lib:FindTaxiByNodeID(Lib.LastTaxi.route[1].nodeID),1}} end
				...
				if taxis and taxis[1] and taxis[1][2]<50 then
					...
					local oldtime,precise = Lib:GetTaxiTripTime(dep_nodeID,arr_nodeID)

					Lib.fc_by_nodeID[dep_nodeID].neighbors[arr_nodeID]=triptime
					ZGV.db.global.taxitimes = ZGV.db.global.taxitimes or {}
					ZGV.db.global.taxitimes[dep_nodeID.."_"..arr_nodeID]=triptime
					local reversed
					...
					if Lib.fc_by_nodeID[arr_nodeID].neighbors[dep_nodeID]==0 then
						...
						ZGV.db.global.taxitimes[arr_nodeID.."_"..dep_nodeID]=triptime
					end
		...
		for dest=1,NumTaxiNodes() do if TaxiNodeGetType(dest)=="REACHABLE" then
			...
			for hop=1,GetNumRoutes(dest) do
				...
				if starttag~=endtag then
					...
					if (fcstart.neighbors[fcend.nodeID]==0 or fcend.neighbors[fcstart.nodeID]==0) and not listed[fcstart.nodeID.."_"..fcend.nodeID] then
						listed[fcstart.nodeID.."_"..fcend.nodeID]=true
	...
	function Lib:OnTimer()
		...
		elseif TaxiFrame and TaxiFrame:IsShown() then
			...
			for routeIndex = 1, GetNumRoutes(slotIndex) do
				...
				if hops[#hops]~=currentPin.taxiNodeData.nodeID then tinsert(hops,currentPin.taxiNodeData.nodeID) end
				tinsert(hops,taxislots[destinationSlotIndex].nodeID)

				local startNode = self:FindTaxiByNodeID(taxislots[sourceSlotIndex].nodeID)
				local endNode = self:FindTaxiByNodeID(taxislots[destinationSlotIndex].nodeID)
				if not startNode then print("What? No start node "..taxislots[sourceSlotIndex].nodeID) return end
				if not endNode then print("What? No end node "..taxislots[destinationSlotIndex].nodeID) return end
