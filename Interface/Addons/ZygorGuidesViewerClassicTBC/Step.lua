...
function Step:ResetCurrentWaypoint()
	ZGV:Debug("&waypoints |cffddaa88Step|r:|cffccff88ResetCurrentWaypoint|r")
...
function Step:CycleWaypoint(delta,nocycle)
	...
	ZGV:Debug("&waypoints |cffddaa88Step|r:|cff88ffffCycleWaypoint|r(|cffffffff%s|r) in step %d having %d goals; cycling (%s) from %d actually %d",(delta>0 and "forward" or "back"),self.num,#self.goals, nocycle and "one-way" or "around", tostring(self.current_waypoint_goal_num),goal_num)
	local goal
	local cycles=0
	...
	repeat
		...
		if goal_num==starting_goal_num then ZGV:Debug("&waypoints - Went full cycle or nowhere, aborting.") return end --full cycle or no change at all, abort
		
		goal=self.goals[goal_num]
		if not goal then ZGV:Debug("&waypoints - Went out of goals!!!") return end

		cycles=cycles+1
		if cycles>50 then ZGV:Debug("&waypoints - Cycling forever, aborting.") return end

	until goal and goal.x and not goal.force_noway and goal:IsVisible()

	ZGV:Debug("&waypoints - Successfully cycling to %d",goal_num)
...
function Step:CycleWaypointTo(goalnum)
	...
	if goal and goal.x and not goal.force_noway and goal:IsVisible() then
		ZGV:Debug("&waypoints |cffddaa88Step|r:|cff88ffccCycleWaypointTo|r(|cffffffff%d|r) ['%s'], way at [%.1f %.1f]",goal.num,goal:GetText(),goal.x*100,goal.y*100)
...
function Step:CycleWaypointFrom(goalnum)
	self.current_waypoint_goal_num = goalnum
	ZGV:Debug("&waypoints |cffddaa88Step|r:|cff88ff88CycleWaypointFrom|r(|cffffffff%d|r)",goalnum)
	self:CycleWaypoint(1,"nocycle")
end
...
function Step:GetDebugDump()
	...
	table.insert(ret,ZGV.QuestDB:ExplainStep())
