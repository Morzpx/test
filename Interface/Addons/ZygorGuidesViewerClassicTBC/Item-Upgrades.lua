...
function Upgrades:ShowEquipmentChangeNotification(slot)
	...
	if ZGV.NotificationCenter then
		...
		if not _G[slottype] then ZGV:Error(("Warning: item found with an invalid type %s (%s)"):format(slottype,new_item.itemlinkfull or new_item.itemlink)) end

		ZGV.NotificationCenter:AddEntry(
			new_item.itemlink,
			L['notifcenter_gear_title'],
			L['notifcenter_gear_equipped']:format(new_item.itemlinkfull,_G[slottype] or slottype or "?"),
			texture,
			texcoords,
			onClick,
			nil,
...
function Upgrades:Equip(item,retry)
	if not item then return end
	local _, _, _, _, _, _, _, _, _, itemID = GetContainerItemInfo(item.bagnum,item.bagslot)
