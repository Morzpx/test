...
function ZGV.QuestDB:MaybeSkipThisGoal(goal)
	...
	if ZGV.db.char.SISdestination and ZGV.db.char.SISquests then
		if ZGV.CurrentGuide.title == end_guide and goal.parentStep.num>=end_step then -- if we are in final guide, don't skip after reaching target step
			return false,"don't skip past target step"
		end
	end
...
function ZGV.QuestDB:ExplainStep(doprint)
	...
	for _,goal in pairs(ZGV.CurrentStep.goals) do
		if goal.action == "accept" then
			...
			local complete = goal:IsComplete()
			out = out .. "Quest "..quest.." "..verbose_status[state].." "..comment.."\n"
			out = out .. " - goal is complete: "..tostring(complete).."\n"
			local s1, s2 = ZGV.QuestDB:MaybeSkipThisGoal(goal)
			out = out .. " - would goal be skipped: "..tostring(s1).." "..s2.."\n"
			if ZGV.ChainsBreadcrumbs[prereqs] then 
				out = out .. "- is a breadcrumb".."\n"
			end
			if ZGV.ChainsSiblings[questid] then
				out = out .. " - has siblings "..table.concat(ZGV.ChainsSiblings[questid],",").."\n"
			end
		...
		for j,questpack in pairs(guide) do
			for quest in pairs(questpack.ids) do
				if not (QuestDB.QuestNamesStored[quest] or QuestDB.QuestNames[quest]) then
					local title = C_QuestLog.GetQuestInfo(quest)
					if title then
