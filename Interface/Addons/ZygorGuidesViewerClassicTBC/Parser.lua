...
local ConditionEnv = {
	...
	races={
		"NightElf","Dwarf","Human","Gnome","Draenei",
		"Orc","Troll","Scourge","Tauren","BloodElf","Undead"
	},
	classes={},
	...
	grouprole = function(role)
		if ZGV.db.profile.showallroles then return true end

		if role=="DPS" then role="DAMAGER" end

		if ZGV.ItemScore.playeristank and role=="TANK" then return true end
		if ZGV.ItemScore.playerishealer and role=="HEALER" then return true end
		return role=="DAMAGER"
	...
	indungeon = function(n)
		local dungid = select(8,GetInstanceInfo())
		return n and dungid==n or dungid>1
	end,
...
tinsert(ZGV.startups,{"Parser unit tests",function(self)
	do -- unit-check ParseMapXYDist
		...
		mapcheck("Stormwind City",function(map,flr,x,y,dist) return map==1453 end)
		...
		mapcheck("12.3,12.8,-2",function(map,flr,x,y,dist) return is(x,0.123) and is(y,0.128) and dist==-2 end)
		mapcheck("Stormwind City,12.3,12.8",function(map,flr,x,y,dist) return map==1453 and is(x,0.123) and is(y,0.128) end)
		mapcheck("Stormwind City,12.3,12.8,2",function(map,flr,x,y,dist) return map==1453 and is(x,0.123) and is(y,0.128) and dist==2 end)
		mapcheck("Stormwind City,12.3,12.8,-2",function(map,flr,x,y,dist) return map==1453 and is(x,0.123) and is(y,0.128) and dist==-2 end)
		...
		-- new syntax
		mapcheck("Stormwind City 12.3,12.8",function(map,flr,x,y,dist) return map==1453 and is(x,0.123) and is(y,0.128) end)
		mapcheck("Stormwind City 12.3,12.8 >5",function(map,flr,x,y,dist) return map==1453 and is(x,0.123) and is(y,0.128) and dist==-5 end)
		...
		mapcheck("Stormwind City/0",function(map,flr,x,y,dist) return map==1453 and flr==0 end)
		...
		mapcheck("Durotar/0 12.3,12.8",function(map,flr,x,y,dist) return map==1411 and is(x,0.123) and is(y,0.128) end)
