...
function Pointer:RefreshDynamicValues()
	do return end -- no pois on classic
...
function Pointer:PointToNextTimer(elapsed)
	...
	and waypoint.m==ZGV.CurrentMapID  -- Try to point-to-next only on current map..? desperate.
	then
...
function Pointer:FindTravelPath(way)
	...
	elseif type(way)=="string" then
		...
		--[[
		-- NEW: if points are in this zone, STOP waypointing. I don't really like this solution, but let's try it for now... --
...
-- C_Timer.NewTicker(1,function() WorldMapFrame.BlackoutFrame:Hide() end)