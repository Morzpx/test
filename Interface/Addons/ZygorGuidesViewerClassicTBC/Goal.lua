			...
			if self.objnum then
				...
				if questGoalData then
					...
					if questGoalData.complete then
						return true, true, num,count,  "quest goal complete"
					else
...
GOALTYPES['create'] = {
	parse = function(self,params)
		...
		if spell then
			...
			if total then
				self.targetid = ZygorGuidesViewer.Professions.SpellToItem[self.spellid]
			...
			self.target,self.targetid = ParseID(item)
		end
	end,
	iscomplete = function(self)
...
GOALTYPES['ontaxi'] = {
	...
	gettext = function(self) return L["stepgoal_intaxi"] end,
}
...
GOALTYPES['offtaxi'] = {
	...
	gettext = function(self) return L["stepgoal_outtaxi"] end,
}
	...
	autoscript = function(self)
		...
		if not (ZGV.db.char.dontshowgrindpopup or GOALTYPES.level.iscomplete(self)) and not (ZGV.db.char.SISguides and ZGV.db.char.SISguides[ZGV.CurrentGuide.title]) and (ZGV.CurrentGuide.type=="LEVELING") then 
			if not ZygorGrindPopup then
				...
				dialog:SetText(L["grind_popup_title"],L["grind_popup_text"])
	...
	if self.targetid then
		...
		if self.action=="kill" then
			--if ZygorGuidesNPCs then
				local target,tooltip=ZGV.Localizers:GetTranslatedNPC(self.targetid,self.target)
				if target then
	...
	elseif self.action=='create' then
		if self.skill then
			...
			if self:IsComplete() then
				text = ("Created %s"):format(COLOR_ITEM(plural(self.spell,self.count or 2)))
				if self.skilllevel then
			...
			-- old create syntax
			text = GenericText(brief,"create",COLOR_ITEM,self.count,ZGV:GetItemInfo(self.targetid),not self.count or self.count==1,self.count and self.count>1,(complete and "_done" or ""))
		end
		--]]
