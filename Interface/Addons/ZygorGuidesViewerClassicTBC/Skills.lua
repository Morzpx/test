...
function Skills:IsDependantKnown(skill)
	...
	if IsSpellKnown(skill) or IsPlayerSpell(skill) or IsSpellKnownOrOverridesKnown(skill) then return true end
	if IsSpellKnown(dep) or IsPlayerSpell(dep) or IsSpellKnownOrOverridesKnown(dep) then return true end
...
function Skills:GetLearnableSkills(level)
				   ...
				   then
					if (sdata.optional==true) or (type(sdata.optional)=="function" and sdata.optional())  then
						if ZGV.db.profile.n_popup_skills_optional then
							...
							table.insert(entry.ranks,{skill=sdata.skill,rank=sdata.rank or "",cost=sdata[2],req=sdata.req,talent=sdata.talent})
						end
					else
	...
	if ZGV.db.profile.n_popup_skills_future then
		for slevel=level+1,max_level do
			local skills=Skills.SkillTraining[classname][slevel]
				...
				for _,sdata in pairs(skills) do
					if (not sdata.talent or IsSpellKnown(sdata.talent) or IsPlayerSpell(sdata.talent)) 
					   ...
					   then
						...
						table.insert(entry.ranks,{skill=sdata.skill,rank=sdata.rank,cost=sdata[2],req=sdata.req,talent=sdata.talent})
					end
				end
			end
		end
