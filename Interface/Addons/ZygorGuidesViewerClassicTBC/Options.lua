...
function ZGV:Options_DefineOptionTables()
	...
	do
		...
		AddOption('gear_selected_class',{
			...
			set = function(i,v) 
				...
				if v == ZGV.ItemScore.playerclassNum then
					ZGV.db.char.gear_selected_build = ZGV.db.char.gear_selected_build  or 1
				else
		...
		AddOption('gear_selected_build',{
			type = "select",
			name="",
			values = function() return ZGV.ItemScore.Builds[ZGV.db.char.gear_selected_class or 1] end,
			get = Getter_Simple,
		...
		for class,classdata in pairs(ZGV.ItemScore.Defaults) do
			for specnum,specdata in pairs(classdata) do
				...
				AddSubgroup(groupname,{
					...
					name = "",
					hidden=function() 
						local visibility = (tonumber(ZGV.db.char.gear_selected_class)==ZGV.ClassToNumber[class])  
							       and (tonumber(ZGV.db.char.gear_selected_build)==specnum)
					...
					AddOption('',{
						...
						width=300,
						hidden=function() 
							local visibility = (tonumber(ZGV.db.char.gear_selected_class)==select(3,UnitClass("player")))  
								       and (tonumber(ZGV.db.char.gear_active_build)==specnum)
	...
	do
		...
		AddOption('profile_reset',{ type = 'execute', func=function(i, value)
			...
			if ZGV.db.char.gear_selected_spec==5 then ZGV.db.char.gear_selected_spec=1 end
