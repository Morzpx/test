...
function ItemScore:OnEvent(event,arg1,arg2,...)
	...
	elseif event == "SKILL_LINES_CHANGED" then -- skills changed, so user may have learned new weapon skill
...
function ItemScore:SetStatWeights(playerclass,playerlevel)
	...
	local active_spec = ZGV.db.char.gear_active_build or 1
	...
	for i,v in pairs(self.ActiveRuleSet.stats) do
		if v>0 then
			if not self.whiteScoreWeight or self.whiteScoreWeight>(v/10) then
				self.whiteScoreWeight = v/10
			end
		end
	end

	if not self.whiteScoreWeight then self.whiteScoreWeight=0.1 end -- in case user statweights are wiped clean
...
function ItemScore:GetItemDetails(itemlink)
	...
	if not (ItemCache[itemlink] and ItemCache[itemlink].stats) then
		...
		-- get whatever we can from GID, or not. GID is now completely batshit crazy
		--[[
		local gistats = GetItemStats(itemlink) or {}
		for statname,statvalue in pairs(gistats) do
			local sanitised = ItemScore:NormaliseStatName(statname)
			if ItemScore.KnownKeyWords[sanitised] then
				stats[sanitised] = tonumber(statvalue)
			end
		end
		--]]
		...
		for num=1,Gratuity:NumLines() do
			...
			if found_level then 
				itemMinLevel = tonumber(found_level) 
				--break -- no hard stats after level requirements line, only stuff that can be false matches. Dream on, stats are no dumped into equip effects
			end

			--if line==" " then break end -- no hard stats after empty line either, see above

			local socket_bonus = line:match( gsub(ITEM_SOCKET_BONUS,"%%s","(.*)"))
			if not socket_bonus then
				for _,statdata in pairs(ItemScore.Keywords) do
					...
					if value then 
						...
						stats[statdata.blizz] = (stats[statdata.blizz] or 0) + tonumber(value)
					end
					if not value and statdata.regex2 then
						local sign,value = line:match(statdata.regex2)
						if sign and not value then value = sign end
						if value then 
							if sign=="-" then value=value*(-1) end
							stats[statdata.blizz] = (stats[statdata.blizz] or 0) + tonumber(value)
				...
				for _,statdata in pairs(ItemScore.KeywordsSpellSchools) do
					local sign,value = line:match(statdata.regex)
					local ovalue
					if sign and not value then value = sign end
					if value then 
		...
		-- blizzard GetItemStats falsely reports school spell damage as regular with value 1 less.
		-- remove the non-schooled version and pray for no false positives
		--[[
		if stats.SPELL_DAMAGE_DONE then
			for _,statdata in pairs(ItemScore.KeywordsSpellSchools) do
				if stats[statdata.blizz] == stats.SPELL_DAMAGE_DONE+1 then
					stats.SPELL_DAMAGE_DONE = nil
					break
				end
			end
		end
...
function ItemScore:GetItemScore(itemlink,verbose)
	...
	if not statweights.ARMOR or statweights.ARMOR==0 then score = score + (item.stats.ARMOR or 0)*self.whiteScoreWeight end
	if not statweights.DAMAGE_PER_SECOND or statweights.DAMAGE_PER_SECOND==0 then score = score + (item.stats.DAMAGE_PER_SECOND or 0)*self.whiteScoreWeight end
...
function ZGV.TestSpellDamage()
	local Gratuity = LibStub("LibGratuity-3.0")

	local items = {
		["item:15971::::::1877:1536996352:1:::::::"] = "Fire",
		["item:4569::::::1837:2076101376:1:::::::"] = "Shadow",
		["item:6564::::::1953:1817980928:1:::::::"] = "Frost",
		["item:2075::::::1799:1556697344:1:::::::"] = "Arcane",
		["item:9776::::::1992:1556697344:1:::::::"] = "Nature",
		["item:15298::::::1913:171304320:1:::::::"] = "Holy"
	}
	...
	for item,result in pairs(items) do
		local res = ZGV.ItemScore:GetItemDetails(item)
		if res then
			if next(res.stats) then
				out[result] = res.stats
			else
				failed = true
			end
		else
		...
		if failed then ZGV:ScheduleTimer(function() ZGV.TestSpellDamage() end,0,1) return end
		...
		for num=1,Gratuity:NumLines() do
			local line=Gratuity:GetLine(num)
			if not line then break end
			table.insert(tips[result],line)
		end
	end

	ZGV:ShowDump({ItemScore=out,Tooltip=tips,Keywords=ZGV.ItemScore.KeywordsSpellSchools})
end
