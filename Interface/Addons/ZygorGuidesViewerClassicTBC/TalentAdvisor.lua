...
function ZTA:HookToTalentFrame()
	...
	hooksecurefunc("TalentFrameTab_OnClick",function() ZTA:ShowTalentSuggestions() end)


	if not self.TalentFrame.advisorbutton then
	...
	self.options = {
		...
		args = {
			...
			zta_hints = {
				...
				disabled = function() return not ZTA:IsCurrentBuildUsable() end,
				order = 7.1,
				_default = true,
			},
			zta_preview = {
...
function ZTA:LoadBuilds()
	if self.db.char.zta_currentBuildKey~=BUILDKEY_NONE then self:SetCurrentBuild(self.db.char.zta_currentBuildKey,false) end
end
...
function ZTA:ShowTalentSuggestions()
	...
	--for tab,panel in ipairs(panels) do
		...
		for talent=1,talents do
			...
			if self.db.profile.zta_preview --[[means: show ranks!! not "preview" in the sense of preview-talents!!]]
			...
			if self.db.profile.zta_hints then
